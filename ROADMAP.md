...
| `009_roles.sql` | `user_roles` table, `bb_my_access()`, admin role controls | ❌ **not yet run** |
...
- 💡 **Power-user heat map** of eating/drinking patterns — **unblocked**: Roles landed in v2.6, so this can be planned whenever it is wanted. Note it is now a *paid-tier* feature by definition rather than a reward for heavy use, which changes who ever sees it.
